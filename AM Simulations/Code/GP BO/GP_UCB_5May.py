...
def GP_BO(max_iters,xlimits,n,d,new_n,m,f,actual_optimum,kernel_init,lcb_gp,label,folder):
    ...
    for iters in range(max_iters):
        ...
        np.random.seed(seed) 
        x = np.random.uniform(low = xlimits[:,0],high = xlimits[:,1],size = (n,d))
        y = f(x).reshape(-1,1)
        ...
        for i in range(n,n+new_n):
            ...
            gpr = GaussianProcessRegressor(kernel=kernel,n_restarts_optimizer=5,random_state=1000*iters + 2*i)
            gpr.fit(x, y)
        ...
        mdic = {"current_opt_DGPSIEI": current_opt,"regret_DGPSIEI": regret, "runtime_DGPSIEI":time_elapsed}
        savemat(folder+label+"_DGPSIEI_"+ str(time.asctime(time.localtime(time.time())))+"_iter"+str(iters)+".mat", mdic)
