...
from GP_EI_tri_stable_corr import GP_BO_EI
...
def main():
    ...
    current_opt,regret,run_time_GP = GP_BO_EI(max_iters,xlimits,n,d,new_n,m,f,actual_optimum,rbf_kernel,lcb_gp,label,folder)

    mdic = {"current_opt_GPEI": current_opt,"regret_GPEI": regret, "runtime_GPEI":run_time_GP}
    savemat(label+"_GPEI_full"+str(time.asctime(time.localtime(time.time())))+".mat", mdic)
